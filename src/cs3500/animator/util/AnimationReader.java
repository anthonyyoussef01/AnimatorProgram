...
public class AnimationReader {
  ...
  private static <Doc> void readSlomo(Scanner s, AnimationBuilder<Doc> builder) {
    int start;
    int end;
    if (s.hasNextInt()) {
      start = s.nextInt();
    } else {
      throw new IllegalStateException("Slomo: Expected a start tick, but no more input available");
    }
    if (s.hasNextInt()) {
      end = s.nextInt();
    } else {
      throw new IllegalStateException("Slomo: Expected an end tick, but no more input available");
    }
    builder.addSlomoInterval(new int[] {start, end});
  }
